...
QUnit.test('Testing calculateSeats function with several sets of inputs', function (assert) {
    assert.equal(App.calculateSeats(2, 5), 10, 'Bonus-1: Tested with two relatively small positive numbers');
    assert.equal(App.calculateSeats(-6, -6), 0, 'Bonus-2: Tested with two negative numbers. Any arguments less than 1 will be set to 1.');
    assert.equal(App.calculateSeats(540, 200), 0, 'Bonus-3: Tested with two large positive numbers. Any arguments greater than 100 will be set to 100.');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.equal(App.calculateSeats(null), 0,  'Bonus-4: Passing in null correctly returns 0');
        //throws( block                                    [, expected ] [, message ] ) 
    assert.equal(App.calculateSeats("Sai Mohan","Vummidi"), 0,  'Bonus-5: Passing in a string correctly returns 0');
    assert.equal(App.calculateSeats(3.4,2.8), 0, 'Bonus -6: The given argument is invalid','Passing in null correctly raises an Error');
});
